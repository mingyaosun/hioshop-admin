                                ...
                                <el-table-column prop="cost2" label="图片" width="200">
                                    <template scope="scope">
                                        <el-upload
                                                ...
                                                :on-success="function (res, file) { return handleUploadGallerySuccess1(res, file, scope.row.id,scope)}"
                    ...
                    this.axios.post('goods/deleteComment', {id: row.id}).then((response) => {
                        if (response.data.errno === 0) {
                            for (const argumentsKey in imgList) {
                                ...
                                //删除评论的照片
                                this.galleryRemove1(file);
                                this.commentData.splice(index, 1);
                            }
                        }
                    })
                } else {
                    //首次添加照片，照片的cid会是空，这里有bug
                    //首次添加时候，删除之前从列表中移除，不入库
                    this.commentData.splice(index, 1);
                    //删掉服务器上的照片
                    //TODO
                    let remoteImgArr = this.commentImgFirst;
...
//                            this.specData = [];
            ...
            handleUploadGallerySuccess1(res, file, cidd, scope) {
                debugger
                let row = scope.row;
                ...
                if (res.key != '') {
                    ...
                    if (cidd > 0) {
                        ...
                        this.axios.post('goods/comimg', info).then((response) => {
                            //走的getcomimgList方法，这个方法是查照片列表
                            that.getGalleryList1(cidd);
                        })
                    } else {
                        //第一次编辑
                        //要获得图片路径，保存时候用
                        //第一次编辑，照片不随表单一起入库，要不然会造成评论图片表的cid为0，后续找不到
                        //图片属于第几条评论
                        //TODO 这里的问题是如果新增时候删除了，需要重新计算数组内容，暂时未实现
                        let key = index + '$index$';
                        that.commentImgFirst.push(key + urlData);
                        console.log(that.commentImgFirst)
                        // this.axios.post('goods/comimg', info).then((response) => {
                        //     //走的getcomimgList方法，这个方法是查照片列表
                        //     that.getGalleryList1(cidd);
                        // })
